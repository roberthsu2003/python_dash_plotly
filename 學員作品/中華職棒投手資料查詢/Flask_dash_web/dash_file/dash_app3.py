...
dash3.layout = html.Div(
    [
        dbc.Container([
            html.Div([
                html.Div([
                    html.Img(src=[cpbl_datasource.img_pic('brothers_index')],width=1000),
                    html.Br(),
...
def search_clickBtn(n_clicks:None | int, inputValue:str):
    ...
    #當clickBtn is None -> 「確定」按鈕沒被按下，網頁剛啟動時
        ...
        print('第一次啟動')
        brothers_data = cpbl_datasource.brothers_data()
        brothers_df = pd.DataFrame(brothers_data,columns=['年份', '所屬球隊', '球員編號', '球員姓名',  '先發次數', '中繼次數', '勝場數', '敗場數', '三振數', '自責分'])

        return brothers_df.to_dict('records'), [{'id':column,'name':column} for column in brothers_df.columns],[]   
...
def selectedRow(selected_rows:list[int]): #傳入list[裡面放int]
        ...
        if len(selected_rows) != 0:
            ...
            idSite:pd.DataFrame = brothers_df.iloc[[selected_rows[0]]]
