...
dash6 = Dash(requests_pathname_prefix="/dash/dragons/", external_stylesheets=[dbc.themes.BOOTSTRAP])

#連結外部css檔
external_stylesheets=['assets/header.css']

dash6.title='味全龍 中華職棒投手資料查詢系統-Chinese Professional Baseball League Pitchers'

current_data = cpbl_datasource.dragons_data()
current_df = pd.DataFrame(current_data,
                          columns=['年份','所屬球隊', '球員編號', '球員姓名', '先發次數', '中繼次數', '勝場數', '敗場數', '三振數', '自責分'])

#layout
dash6.layout = html.Div(
    [
        dbc.Container([
            html.Div([
                html.Div([
                    html.Img(src=[cpbl_datasource.img_pic('dragons_index')],width=1200),
                    html.Br(),
            ...
            style={'justify-content':'center', 'display':'flex', 'padding':'1rem', 'margin':'2rem'}),
                ...
                html.A(id='hawks',className='team_logo',href='/dash/index', children=[
                    html.Img(src=[cpbl_datasource.team_logo('hawks')], width='160px', height='160px'),
                 ],style={'text-decoration': 'none','margin-left':'2rem'}),html.Br(),

                html.A(id='home',className='team_logo',href='/dash/index', children=[
